...
策略模式的Strategy类层次为Context定义了一系类的可供重用的算法或者行为，继承有助于截取出这些算法中的公共功能。同时策略模式的优点是简化了单元测试，因为每个算法都有自己的类，可以通过自己的接口单独测试。

策略模式就是用来封装算法，但在实践中我们可以发现用他来封装几乎任何类型的规则，只要在分析过程中听到需要在不同实践应用不同的业务规则，就可以考虑使用策略模式处理这种变化的可能性
...
![](https://img-blog.csdnimg.cn/img_convert/d5e1aa72440f93d71987a09d530580a5.png)
![](https://img-blog.csdnimg.cn/img_convert/3912269a638122ce1052e36a0a28c41c.png)
## 模式分析
